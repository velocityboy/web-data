...
class HttpResponseItem extends ScrollView {
  ...
  appendResponse(result) {
    ...
    if (result.error !== null) {
      ...
      let imageExt = this.imageExtensions[response.headers['content-type']];
      if (imageExt) {
        this._appendImage(bodyDiv, result.body, imageExt);
      } else {
        let editor = document.createElement('atom-text-editor');
        let decoder = new StringDecoder('utf8');
        editor.getModel().setText(decoder.write(result.body));
  ...
  _appendImage(container, body, ext) {
    temp.track();
    temp.open({suffix: ext}, (err, info) => {
      if (!err) {
        fs.write(info.fd, body, 0, body.length, null, (err) => {
          fs.close(info.fd);
